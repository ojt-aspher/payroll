<template>
  ...
  >
    ...
    >
    <v-card-subtitle
      >Compute salary for the employees.</v-card-subtitle
    >
    <v-card-text>
      <v-row>
        <v-col cols="12" md="9">
          ...
          <v-row dense> -->
            ...
            <v-col cols="12" md="4">
              <v-text-field
                type="number"
                label="No. of Non-Working Days"
                outlined
                :disabled="totalDaysActive === 0"
                v-model="totalDaysNonWorking"
              ></v-text-field>
            </v-col>
          <!-- <v-col cols="12" md="4">
              <v-text-field
                :label="`No. of non-working days${
                  totalDaysNonWorking > 0 ? `(${formatMoney(nonWorkingDeduction)})` : ''
                }`"
                outlined
                type="number"
                v-model="totalDaysNonWorking"
...
export default {
  ...
  computed: {
    ...
    totalDaysActive() {
      const days = this.totalDaysBetweenTwoDatePeriods - this.totalDaysAbsent - this.totalDaysNonWorking;
    ...
    nonWorkingDeduction() {
      let salary = 0;
      if (this.totalDaysActive === 0) return salary;
      salary = this.employeeDailySalary * this.totalDaysNonWorking;
      return salary;
    },
    ...
    sssDeduction() {
      ...
      const contributionPercent = 0 //0.0363;
    ...
    pagIbigDeduction() {
      let deduction = 0;
      if (this.totalDaysActive === 0) return deduction;
      const contributionPercent = 0 //0.02;
      deduction = this.basicSalary * contributionPercent;
      return deduction.toFixed(2);
    },
    ...
    philhealthDeduction() {
      let deduction = 0;
      if (this.totalDaysActive === 0) return deduction;
      const contributionPercent = 0 //0.0275;
      deduction = this.basicSalary * contributionPercent;
      return deduction.toFixed(2);
    },
    ...
    birTaxPercentage() {
      ...
      if (basicSalary >= 25000.01 && basicSalary <= 40000) taxPercentage = 0//0.2;
      if (basicSalary >= 40000.01 && basicSalary <= 80000) taxPercentage = 0//0.25;
      if (basicSalary >= 80000.01 && basicSalary <= 200000) taxPercentage = 0//0.3;
      if (basicSalary >= 200000.01 && basicSalary <= 800000)
        taxPercentage = 0.32;
      if (basicSalary >= 800000.01) taxPercentage = 0//0.35;
    ...
    basicSalaryWithoutAbsent() {
      let salary = 0;
      if (this.totalDaysActive === 0) return salary;
      salary = this.employeeDailySalary * this.totalDaysBetweenTwoDatePeriods - this.nonWorkingDeduction;
      return salary;
    },
